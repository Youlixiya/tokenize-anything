...
class TransformerCache(nn.Module):
    ...
    def forward_flash(self, mixer, q, k, v, attn_masks=None):
        ...
        if cache_k is not None or cache_v is not None:
            ...
            cache_seqlens = self.cache_dict["seq_lens"][0]
            ...
            if cache_seqlens > 0:
                attn_mask = torch.ones((1, seq_lens, k.shape[1]), device=q.device)
                if seq_lens > 1:
    ...
    #             (seq_lens, seq_lens), float("-inf"), device=q.device
    #         )
    ...
    #             torch.zeros((seq_lens, seq_lens), device=q.device),
    #             attn_mask
    #         ]).to(dtype=torch.bool)
