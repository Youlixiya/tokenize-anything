...
class Attention(nn.Module):
    ...
    def forward(self, q, k, v):
        q = self.q_proj(q).view((-1, q.size(1), self.num_heads, self.head_dim)).transpose(1, 2)
        k = self.k_proj(k).view((-1, k.size(1), self.num_heads, self.head_dim)).transpose(1, 2)
        v = self.v_proj(v).view((-1, v.size(1), self.num_heads, self.head_dim)).transpose(1, 2)
...
class BoxDecoder(nn.Module):
    ...
    def __init__(self, depth, embed_dim, num_heads):
        ...
        # self.iou_token = nn.Embedding(1, embed_dim)
        # self.sem_tokens = nn.Embedding(self.num_mask_tokens, embed_dim)
        self.box_tokens = nn.Embedding(1, embed_dim)
        # self.output_conv = nn.Sequential(
        #     nn.ConvTranspose2d(embed_dim, embed_dim // 4, 2, 2),
        #     TransposedLayerNorm(embed_dim // 4),
        #     nn.GELU(),
        ...
        #     nn.GELU(),
        # )
        self.box_pred = Predictor(embed_dim, 4)
        # self.iou_pred = Predictor(embed_dim, self.num_mask_tokens)
        # self.sem_pred = Predictor(embed_dim, sem_embed_dim, 1024)

    def get_outputs(self, inputs):
        img_embeds = inputs["img_embeds"]
        box_embeds = inputs["box_embeds"]
        ims_per_batch = img_embeds.size(0)
        box_per_batch = box_embeds.size(0)
        img_embed_size = img_embeds.shape[2:-1]
        # Prepare query.
        query = self.box_tokens.weight.unsqueeze(0).expand(box_per_batch, -1, -1)
        # query = tokens.unsqueeze_(0).expand(prompts_per_batch, -1, -1)
        query = torch.cat((query, box_embeds), dim=1)
        # num_tokens = query.shape[1] - sparse_embeds.shape[1]
        # Prepare key.
        key = img_embeds.expand(-1, box_per_batch // ims_per_batch, -1, -1, -1)
        key = key.flatten(0, 1).flatten(1, 2)
        # Decode.
        query, key = self.transformer(query, key, query, inputs["img_pos"])
        # Upscale key.
        ...
        # sem_tokens = tokens[: self.num_mask_tokens]
        # Predict.
        box_pred = F.sigmoid(self.box_pred(box_tokens))
        outputs = {"box_pred": box_pred}
        # mask_pred = torch.stack(mask_pred, dim=1) @ mask_embeds
        # mask_pred_size = list(4 * embed_size for embed_size in img_embed_size)
        # mask_pred = mask_pred.view([-1, self.num_mask_tokens] + mask_pred_size)
        # outputs = {"iou_pred": self.iou_pred(iou_tokens), "mask_pred": mask_pred}
